...
- **Behavior.** Zoom is anchored at the pointer (the world point under the cursor stays under the cursor). Clamped to a min/max range (e.g. 10% to 800%).
...
- **Performance.** Outgoing cursor position throttled to ~30 to 60ms on the Awareness channel; receivers interpolate between samples for smoothness.
- **States.** A user with no pointer over the canvas has `cursor: null` (no ghost cursor shown). On disconnect, the cursor disappears via Awareness timeout, so no stale cursors remain.
